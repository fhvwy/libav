...
static int vaapi_encode_mpeg2_init_sequence_params(AVCodecContext *avctx)
{
...

    {
        ...
        AVRational fr, error, best_error = { INT_MAX, 1 };
        int i, n, d, cmp;
        ...
        for (i = 1; i < FF_ARRAY_ELEMS(frame_rate_table); i++) {
            if (av_cmp_q(priv->frame_rate, frame_rate_table[i]) == 0) {
                sh->frame_rate_code        = i;
                se->frame_rate_extension_n = 0;
                se->frame_rate_extension_d = 0;
            ...
            for (n = 1; n <= 4; n++) {
                for (d = 1; d <= 32; d++) {
                    fr = av_mul_q(frame_rate_table[i],
                                  (AVRational) { n, d });
                    cmp = av_cmp_q(fr, priv->frame_rate);
                    if (cmp == 0) {
                        sh->frame_rate_code        = i;
                        se->frame_rate_extension_n = n - 1;
                        se->frame_rate_extension_d = d - 1;
                        goto found_frame_rate;
                    }
                    if (cmp < 0)
                        error = av_div_q(priv->frame_rate, fr);
                    else
                        error = av_div_q(fr, priv->frame_rate);
    ...
    found_frame_rate:
        av_log(avctx, AV_LOG_DEBUG, "frame_rate: code=%d n=%d d=%d.\n",
               sh->frame_rate_code,
               se->frame_rate_extension_n, se->frame_rate_extension_d);
    }
