...
#define CROP(border) do { \
        if (ctx->crop_ ## border >= 0) { \
            sps->conf_win_ ## border ## _offset = ctx->crop_ ## border; \
...
static int h265_metadata_filter(AVBSFContext *bsf, AVPacket *out)
{
    ...
    for (i = 0; i < au->nb_units; i++) {
        if (au->units[i].type == HEVC_NAL_VPS)
            h265_metadata_update_vps(bsf, au->units[i].content);
        if (au->units[i].type == HEVC_NAL_SPS)
            h265_metadata_update_sps(bsf, au->units[i].content);
    }
